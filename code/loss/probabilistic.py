...
def crps(pred: torch.Tensor, pred_sigma: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    """
    ...
    pred_sigma: torch.Tensor. Tensor containing predicted standard deviations.
    ...
    loc = (target - pred) / torch.sqrt(pred_var + eps)
    ...
    return torch.sqrt(pred_var) * (loc * (2. * pphi - 1.) + 2 * phi - 1. / np.sqrt(np.pi))
...
class CRPS(torch.nn.Module):
    ...
    def __call__(self, pred: torch.Tensor, pred_sigma: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        torch.Tensor. Mean CRPS over the batch.
        """
        return crps(pred, pred_sigma, target)
