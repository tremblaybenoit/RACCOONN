...
def main(config: DictConfig) -> None:
    ...
    bt_forward = instantiate(config.data.stage.predict.variables.bt_forward.load)
    bt_forward_stdev = instantiate(config.data.stage.predict.variables.bt_forward_stdev.load)

    # Load test set references
    logger.info("Load test set references...")
    bt_crtm = instantiate(config.data.stage.predict.variables.bt_crtm.load)
    n_channels = bt_crtm.shape[1]

    # Create masks and compute rmse by condition
    mask_dict = {
        'clear': instantiate(config.data.stage.predict.variables.clear_mask.load),
        'cloud': instantiate(config.data.stage.predict.variables.cloud_mask.load),
        'day': instantiate(config.data.stage.predict.variables.daytime_mask.load),
        'night': instantiate(config.data.stage.predict.variables.nighttime_mask.load)
    }
    colors_dict = {
        'clear': '#D81B60',
        'cloud': '#1E88E5',
    ...
    save_plot(fig2, config.paths.run_dir + '/bt_crtm_predict_rmse_bars.png')
