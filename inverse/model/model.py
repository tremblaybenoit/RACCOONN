...
class HydraResidualMLP3(nn.Module):
    def __init__(
            self,
            input_layer: DictConfig,
            hidden_layer: DictConfig,
        ...
        for l in range(hidden_n_layers):
            ...
            # If injecting coords, the input to the block increases
            if inject_coords_hidden and l > 0:
                block_cfg.in_features += input_layer.out_features  # Account for concatenated encoding
                # If residual, adjust out_features
            if block_cfg._target_ == 'inverse.model.model.SirenResidualBlock':
                block_cfg.out_features = block_cfg.in_features
        ...
        for _ in range(output_n_heads):
            ...
            for l in range(output_n_layers):
                ...
                if inject_coords_output:
                    l_cfg.in_features = head_dim + input_layer.out_features
                else:
                    l_cfg.in_features = head_dim
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        outputs = []
        for head in self.output_layers:
            for l, layer in enumerate(head):
                if l == 0:
                    h_input = torch.cat([h, z], dim=-1) if self.inject_coords_output else h
                elif l < len(head)-1:
                    h_input = torch.cat([h_input, z], dim=-1) if self.inject_coords_hidden else h_input
                h_input = layer(h_input)
            outputs.append(h_input)
...
class PINNverseOperator(BaseModel):
    ...
    def _logging(self, stage: str, loss: dict, input: dict, target: dict, pred: dict) -> None:
        ...
        if 'obs' in loss:
            self.log(f"{stage}_loss_obs", loss['obs'].mean(), on_epoch=True, prog_bar=stage == 'valid', logger=logger_flag)
            if loss['obs'].ndim == 2:
...
class PINNverseOperatorPrecondition(PINNverseOperator):
    def __init__(self, lanczos_buffers: DictConfig, optimizer: DictConfig = None, loss_func: DictConfig = None,
                 lr_scheduler: DictConfig = None, architecture: DictConfig = None,
                 parameters: DictConfig = None):
        """ Initialize model.
        ...
        # Add metrics
        self.metrics['prof_lanczos'], self.metrics['prof_target_lanczos'], self.metrics['prof_background_lanczos'] = {}, {}, {}

        # Lanczos Buffers
        lb = instantiate(lanczos_buffers)
        # Background Mean: (V*L,)
        self.register_buffer('x_b', torch.tensor(lb['x_b'], dtype=torch.float32))
        # Per-feature Standard Deviation: (V*L,)
        self.register_buffer('std_devs', torch.tensor(lb['std_devs'], dtype=torch.float32))
        # Correlation Basis Matrix: (V*L, n_modes)
        self.register_buffer('L_corr', torch.tensor(lb['L_corr'], dtype=torch.float32))
    ...
    def forward(self, x: dict) -> dict:
        ...
        # 2. Lanczos Coefficient Prediction (whitened coefficients)
        u = self.model(inputs)
        # 3. Correlation-to-Physical Reconstruction
        # u @ L_corr.T gives the "innovation" in normalized correlation space
        innovation_corr = torch.matmul(u, self.L_corr.T)

        # Rescale by physical standard deviations and add background mean
        # (Batch, V*L) * (V*L,) + (V*L,)
        prof_flat = (innovation_corr * self.std_devs) + self.x_b

        # 4. Final Reshape to (Batch, Variables, Levels)
        prof = prof_flat.view(-1, self.n_prof, self.n_levels)

        return {
            'prof': prof,
            'prof_lanczos': u  # These are the 'u' predicted by MLP
        }
