...
try:
    ...
    checkpoint_path = os.path.abspath(os.path.join(os.path.dirname(__name__), 'forward/model/checkpoints/model_white_000.ckpt'))
    config_path = os.path.join(get_config_path(), 'model/forward_emulator_white.yaml')
...
class VarLoss0(torch.nn.Module):
    ...
    def __call__(self, pred: dict, target: dict) -> tuple[dict, torch.Tensor]:
        ...
        if self.pressure_filter is not None:
            ...
            pressure_filter = torch.ones_like(pred['prof_white'], dtype=torch.bool, device=pred['prof_white'].device)
    ...
    def __call__(self, pred: dict, target: dict) -> tuple[dict, torch.Tensor]:
        ...
        if self.loss_model is not None:
            if isinstance(self.loss_model, (DiagonalQuadraticForm, DiagonalQuadraticHuberForm)):
                if self.pressure_filter is not None:
                    loss['model'] = self.loss_model(pred['prof_white'][:, pressure_filter],
                                                    target['prof_white_background'][:, pressure_filter],
        ...
        # Boundary condition losses (where the variance is zero)
        if self.loss_bcs is not None and self.pressure_filter is not None:
            loss['bcs'] = self.loss_bcs(pred['prof_white'][:, ~pressure_filter], target['prof_white'][:, ~pressure_filter])
            # Total
            loss['total'] += self.lambda_bcs * loss['bcs'].mean()
