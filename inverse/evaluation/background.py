...
def main(config: DictConfig) -> None:
    ...
    config1 = config.data.stage.train.vars.prof_background
    config1.normalization._target_ = 'data.transformations.min_max'
    config1.normalization.axis = 1
    prof_background_train1 = load_var_and_normalize(config1)
    ...
    config2 = config.data.stage.train.vars.prof_background
    config2.normalization._target_ = 'data.transformations.min_max'
    config2.normalization.axis = None
    prof_background_train2 = load_var_and_normalize(config2)
    ...
    config3.normalization._target_ = 'data.transformations.mean_stdev'
    config3.normalization.axis = None
    prof_background_train3 = load_var_and_normalize(config3)
    ...
    prof_background = np.concatenate([prof_background_train1, prof_background_train2, prof_background_train3], axis=1)
    prof_mean = np.mean(prof, axis=0)
    prof_stdev = np.std(prof, axis=0)
    prof_background_mean = np.mean(prof_background, axis=0)
    prof_background_stdev = np.std(prof_background, axis=0)
    breakpoint()
